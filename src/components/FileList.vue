...
<script>
  ...
  export default {
    ...
    methods: {
      downloadFile(item) {
        window.open(`${process.env.API_BASE}/api/file/download?id=${item.id}`)
      },
      async deleteFile(item) {
        this.$confirm('您确定删除此文件吗？','提示',{
          ...
          let res = (await axios.get(`${process.env.API_BASE}/api/file/delete?id=${item.id}`,{withCredentials:true})).data;
