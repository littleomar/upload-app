  ...
  export default {
    ...
    async beforeRouteEnter(to,from,next) {
      ...
      if (user) {
        if (user.role === 0) {
          next( async vm => {
            let res = (await axios.get(`${process.env.API_BASE}/api/user/query?email=${vm.$route.params.email}`,{withCredentials:true})).data;
            if (res.code === 0) {
    ...
    methods: {
      async modifyUser() {
        ...
        let queryRes = (await axios.get(`${process.env.API_BASE}/api/user/verify?realname=${this.user.realname}&email=${this.user.email}`)).data;
        if (!queryRes) {
          this.$message({
            type: 'error',
            message: '服务器错误'
      ...
      removeUser() {
        this.$confirm('您确定要删除此用户吗？删除用户后连同该用户上传的文件也一并删除且将无法找回, 是否继续?', '提示', {
          ...
          let removeRes = (await axios.get(`${process.env.API_BASE}/api/user/admin/removeUser?email=${this.user.email}`)).data;
          if (!removeRes) {
            this.$message({
              type: 'error',
              message: '服务器错误!'
    ...
    async beforeMount() {
      let res = (await axios.get(`${process.env.API_BASE}/api/admin/userDetail?email=${this.$route.params.email}`,{withCredentials:true})).data;
      if (!res) return new Error('服务器错误');
      ...
      let fileRes = (await axios.get(`${process.env.API_BASE}/api/file/list?email=${this.$route.params.email}`,{withCredentials:true})).data;
