  ...
  export default {
    ...
    data() {
      ...
      let validateEmail = async (rule, value, callback) => {
        ...
        let emailRes = (await axios.get(`${process.env.API_BASE}/api/user/verify?email=${this.register.email}`)).data;
        if (emailRes.code === 1) {
      ...
      let validateCode = async (rule, value, callback) => {
        ...
        let codeRes = (await axios.get(`${process.env.API_BASE}/api/user/verify?code=${this.register.code}&email=${this.register.email}`)).data;
        if (codeRes.code === 1) {
      ...
      let validateRealName = async (rule, value, callback) => {
        ...
        let nameRes = (await axios.get(`${process.env.API_BASE}/api/user/verify?realname=${this.register.realName}`)).data;
        if (nameRes.code === 1) {
    ...
    methods: {
      sendEmail(e) {
        ...
        this.$refs['register'].validateField('email',async valid => {
          if (valid) {
            ...
            let codeRes = (await axios.get(`${process.env.API_BASE}/api/user/code?email=${_self.register.email}`)).data;
            if (codeRes.code === 1) {
              return false;
            }
            let time = 60;
      ...
      async submitForm (){
        ...
        let registerRes = (await axios.post(
          `${process.env.API_BASE}/api/user/register`,
          {
